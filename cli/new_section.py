...
def run(args: List[Any]):
    ...
    print(
        f"[*] Output {injector.output} expanded by {shellcode_size} ({expanded_size} actual) bytes"
    )
    print("-- Creating New Section --")
    injector.manager = injector.manager.create_new_section(injector.shellcode, ".extra")
    print(f"[*] Finished: New Section created called '.extra'")


def apply(subparser: _SubParsersAction) -> ArgumentParser:
    parser = subparser.add_parser(
        "new-section", help="add a new section (copies and expands source file)"
    )
